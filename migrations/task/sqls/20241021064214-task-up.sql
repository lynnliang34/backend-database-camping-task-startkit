...
INSERT INTO "CREDIT_PACKAGE" (name, credit_amount, price) -- 新增資料到 CREDIT_PACKAGE 資料表，指定欄位 name, credit_amount, price
VALUES -- 要新增資料的對應值
    ('7 堂組合包方案', 7, 1400),
    ('14 堂組合包方案', 14, 2520),
    ('21 堂組合包方案', 21, 4800);
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id , purchased_credits, price_paid) -- 新增資料到 CREDIT_PURCHASE 資料表
VALUES -- 要新增資料的對應值
(
 (SELECT id FROM "USER" WHERE name = '王小明'), -- 子查詢 USER 資料表 name = '王小明' 的 id
 (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案'), -- 子查詢 CREDIT_PACKAGE 資料表 name = '14 堂組合包方案' 的 id
 (SELECT credit_amount FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案'), -- 子查詢 CREDIT_PACKAGE 資料表 name = '14 堂組合包方案' 的 credit_amount
 (SELECT price FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案') -- 子查詢 CREDIT_PACKAGE 資料表 name = '14 堂組合包方案' 的 price
),
(
...
INSERT INTO "COACH" (user_id, experience_years) -- 新增資料到 COACH 資料表，指定欄位 user_id, experience_years
VALUES -- 要新增資料的對應值
    ((SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'), 2), -- 子查詢 USER 資料表 email = 'lee2000@hexschooltest.io' 的 id
...
INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id) -- 新增資料到 COACH_LINK_SKILL 資料表，指定欄位 coach_id, skill_id
VALUES -- 要新增資料的對應值
(
 (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')), -- 子查詢 COACH 資料表 user_id = (子查詢 USER 資料表 email = 'lee2000@hexschooltest.io' 的 id) 的 id
 (SELECT id FROM "SKILL" WHERE name = '重訓') -- 子查詢 SKILL 資料表 name = '重訓' 的 id
),
(
...
-- 3-3 修改：更新教練的經驗年數，資料需求如下：
    -- 1. 教練`肌肉棒子` 的經驗年數為3年
    -- 2. 教練`Q太郎` 的經驗年數為5年
UPDATE "COACH" -- 更新 COACH 資料表
SET experience_years = 3 -- 將 experience_years 欄位的值改為 3
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io'); -- 篩選出指定 user_id (子查詢 USER 資料表 email = 'muscle@hexschooltest.io' 的 id)
...
INSERT INTO "SKILL" (name) -- 新增資料到 SKILL 資料表，指定欄位 name
VALUES ('空中瑜伽'); -- 要新增資料的對應值

DELETE FROM "SKILL" -- 刪除 SKILL 資料表中的資料
WHERE name = '空中瑜伽'; -- 刪除的資料條件
...
INSERT INTO "COURSE"(user_id,skill_id,name,start_at,end_at,max_participants,meeting_url) -- 新增資料到 COURSE 資料表
VALUES(
